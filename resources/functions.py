...
def verify_is_file(target_file_path: Path | str) -> bool:
...
def verify_file_access(target_file_path: Path | str) -> bool:
    ...
    if not os.access(target_file_path, os.R_OK):
        console.print(
            ...
            f"permissions for {target_file_path.name}"
            )
        return False
    else:
        return True
...
def get_password() -> bytearray:
    ...
    while True:
        ...
        if validate_password(password_bytes):
