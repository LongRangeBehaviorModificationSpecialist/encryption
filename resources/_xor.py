...
class XORClass:
    ...
    def get_xor_key_prompt() -> str:
        return Prompt.ask("""[bright_white]
...
[!] Failed to encrypt {target_file_path.name} : {e}""")
    ...
    def xor_encrypt_files_in_folder(
        self, target_dir_path: Union[Path, str], xor_key: str
    ) -> List[Path]:
    ...
    def get_xor_encryption_choice(self) -> None:
        ...
        while True:
            ...
            try:
                ...
                if xor_encryption_choice == "r":
                    self.return_to_main_menu()
                    return

                if xor_encryption_choice == "q":
                    Functions.exit_application()
                    return

                xor_key = XORClass.get_xor_key_prompt(self)

                if xor_encryption_choice == "1":
                    message = self.get_message_to_xor()
                    XORClass.xor_encrypt_msg(
                        self, xor_key=xor_key, message=message
                    )

                elif xor_encryption_choice == "2":
