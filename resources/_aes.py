...
from resources.prompts import (
    show_main_app_menu,
    show_aes_encryption_menu,
    show_aes_decryption_menu
)
...
class AES:
    ...
    def aes_process_files_in_folder(
            self,
            target_dir: Path | str,
            password: str,
            action: str,
            ...
            if action == "decrypt":
                ...
                if recursive:
                    ...
                    all_files = [
                        f for f in target_dir.rglob("*")
                        if f.is_file()
                        and f.suffix.lower() not in self.EXCLUDE_EXT_LIST]
                else:
                    all_files = [
                        f for f in target_dir.iterdir()
                        if f.is_file()
        ...
        for file_path in all_files:
            try:
                processed_path = self.aes_process_single_file(
        ...
        if results['success']:
            ...
            processed_paths = [item['encrypted'] for item in results['success']]
    ...
    @staticmethod
    def get_aes_action_choice(action: str) -> None:
        ...
        if action == "encrypt":
            ...
            match aes_encryption_choice:
                ...
                case "2":
                    ...
                    AES.aes_process_files_in_folder(
                        action=action,
